...
public class WstgMapperData {
    ...
    /**
     * Fetches the WSTG checklist JSON from the given URL and returns a populated instance.
     *
     * @throws IOException if the network request fails or returns a non-200 status
     */
    public static WstgMapperData fetchFromUrl(String url) throws IOException {
        HttpClient client =
        ...
        try {
            ...
            throw new IOException("Interrupted while fetching WSTG checklist from " + url, e);
        }
        if (response.statusCode() != 200) {
            throw new IOException(
                    "HTTP " + response.statusCode() + " fetching WSTG checklist from " + url);
        }
        try (InputStream is = response.body()) {
            ObjectMapper mapper = new ObjectMapper();
            WstgTestsRoot root = mapper.readValue(is, WstgTestsRoot.class);
            List<WstgCategory> loaded = root.getCategories();
            WstgMapperData newData = new WstgMapperData(loaded != null ? loaded : List.of());
            LOGGER.debug(
                    "Fetched {} WSTG tests across {} categories from {}.",
                    newData.testById.size(),
                    newData.categories.size(),
                    url);
            return newData;
        }
    }
